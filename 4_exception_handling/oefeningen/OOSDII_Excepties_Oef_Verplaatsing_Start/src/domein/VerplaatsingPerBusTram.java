...
public class VerplaatsingPerBusTram extends Verplaatsing
{
    ...
    public final void setLijnnr(int lijnnr) {
        if (lijnnr < 0) throw new IllegalArgumentException("Lijnnummer moet positief zijn!");
        this.lijnnr = lijnnr;
    }
    ...
    public String toString() {
        return String.format("%s met %s%d", super.toString(),(isStadslijn() ? "stads" : "") + (isBus() ? "bus " : "tram "), lijnnr);
    }
    @Override
    public double berekenPrijs() {
        int aantalHaltes = (int) Math.ceil(getAantalKm());
        double prijs = aantalHaltes * PRIJS_PER_HALTE * 2;
        if (isStadslijn()) prijs *= 0.8;
        return prijs;
    }    
}
