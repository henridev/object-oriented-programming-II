...
public class VerplaatsingPerAuto extends Verplaatsing
{
    ...
    public final void setBenzineprijs(double benzineprijs) {
        if (benzineprijs < 0.5 || benzineprijs > 2) throw new IllegalArgumentException("De benzineprijs moet tussen 0.5 en 2 euro liggen");
        this.benzineprijs = benzineprijs;
    }
    
    public final void setVerbruik(double verbruik) {
        if (verbruik < 0.02 || verbruik > 0.07) throw new IllegalArgumentException("Het verbruik moet tussen de 0.02 en de 0.07 liter per km liggen");
