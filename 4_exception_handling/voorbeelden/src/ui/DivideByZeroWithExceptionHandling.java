	...
	public static void main(String[] args) {
		...
		do {
			try {
				...
				System.out.printf("%nResultaat: %d / %d = %d%n", teller, noemer, result);
				blijvenHerhalenFlag = false;
			} catch (InputMismatchException inputMismatchException) {
				System.err.printf("%nException: %s%n", inputMismatchException);
				scanner.nextLine();
				System.out.printf("De invoer moeten integere getallen zijn. Probeer opnieuw.%n%n");
			} catch (ArithmeticException arithmeticException) {
				System.err.printf("%nException: %s%n", arithmeticException);
				System.out.printf("Het cijfer 0 kan geen noemer zijn. Probeer opnieuw.%n%n");
